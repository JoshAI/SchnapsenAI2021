...
def create_dataset(path, player=ml.Bot(), games=2000, phase=2):
...
#create_dataset(options.dset_path, player=ml_evo.Bot(theta=x), games=2000)
...
parser.add_argument("-o", "--overwrite",
                    dest="overwrite",
                    action="store_false",  # added flase
...
options = parser.parse_args()
    ...
    #create_dataset(options.dset_path, player=ml_evo.Bot(theta=), games=2000)  # ml_evo
    #create_dataset('rdeep_dataset.pkl', player=rdeep.Bot(), games=2000) # rdeep
    #create_dataset('rand_dataset.pkl', player=rand.Bot(), games=2000) # rand
    #create_dataset('bully_dataset.pkl', player=bully.Bot(), games=2000) # bully
    #create_dataset('ml_dataset.pkl', player=ml.Bot(), games=2000) # ml with rdeep model
...
if options.train:
    ...
    with open('/complete_dataset.pkl', 'rb') as output:
        data, target = pickle.load(output)
    ...
    # # learner2 = LogisticRegression() ###########

    model = learner.fit(data, target)
    # model2 = learner2.fit(data, target) ###########
    ...
    joblib.dump(model, "./bots/ml_evo/complete_model.pkl")
    #joblib.dump(model3, "./bots/ml/model3.pkl")
